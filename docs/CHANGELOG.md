...
- **`↑↓` 버튼을 없앴다.** 순서 바꾸기는 드래그 핸들이 맡고, 행에는 드래그로 할 수 없는 두 가지
  `+`(다음에 삽입)와 `×`(삭제)만 남는다. 키보드만으로는 순서를 바꿀 수 없게 되지만, 이력 재정렬은
  드문 조작이라 화면이 정리되는 쪽을 택했다.
- 버튼을 둘로 줄여도 결절 열이 좁아 행이 여전히 두 줄로 접혔다. 행마다 반복되던 `mm`을 캡션으로
  올려(`Earlier exams (oldest first, mm)`) **한 줄에 들어가게** 했다. 단위는 어차피 모든 행이 같다.
- 회귀 테스트는 화살표가 사라진 것을 확인하고, 삽입·삭제·체인 순서는 계속 검사한다.

## ver2.0015

- **크기 이력을 드래그해서 순서를 바꾼다.** 행 왼쪽의 `\u2261` 핸들을 잡고 끌면 된다.
  `↑↓` 버튼은 그대로 둔다 — 키보드로 조작할 수 있어야 하고, 세 줄짜리 목록에서는 버튼이 더 정확하다.
- 드래그 중에는 **DOM 노드만 옮기고 상태는 놓을 때 한 번만 쓴다.** 드래그 도중 재렌더하면 행이
  포인터 아래에서 사라진다 — 다이어그램 드래그에서 이미 겪은 문제다 (HANDOFF의 `lnDragActive` 항목).
- 포인터 이벤트를 쓰므로 마우스와 터치가 같은 코드로 동작한다. 핸들에 `touch-action: none`을 줘서
  태블릿에서 끌 때 화면이 같이 스크롤되지 않는다.
- 이동/해제 리스너는 **document에 건다.** `setPointerCapture`를 핸들에 걸었더니 캡처가 유지되지
  않아 이동 이벤트가 커서 아래의 다른 행으로 흩어졌다 (테스트에서 잡힘).
- 회귀 테스트 (59 → 60): 실제 마우스로 첫 행을 아래로 끌어 순서가 바뀌고 판독문 체인이 따라오는지.
  터치 포인터(`pointerType: 'touch'`)로도 재정렬되는 것을 별도로 확인했다.

## ver2.0014

- **크기 이력의 순서를 바꾸고 중간에 끼워 넣을 수 있다.** 행마다 버튼 네 개가 붙었다:
  `↑` 앞으로 · `↓` 뒤로 · `+` 이 행 **다음에** 빈 행 삽입 · `×` 삭제.
  맨 위 행의 `↑`와 맨 아래 행의 `↓`는 비활성화된다.
- 이력에는 날짜가 없어 **순서가 곧 시간 정보**다. 그래서 순서 편집이 없으면 잘못 들어온 순서를
  바로잡을 방법이 없었고, 나중에 기억난 검사를 중간에 넣을 수도 없었다.
- 맨 끝 추가 버튼은 이력이 있을 때 `+ Earlier size (at the end)`로 문구가 바뀐다 — 중간 삽입은
  행의 `+`, 끝 추가는 이 버튼이라는 것이 드러나게.
- 버튼 네 개를 한 묶음(`.hist-btn-group`)으로 감쌌다. 결절 열이 좁아 줄이 접힐 때 2+2로 갈라지던
  것을 한 덩어리로 접히게 했다. 높이는 `--h-chip`을 써서 높이 사다리에 raw px가 늘지 않는다.
- 회귀 테스트 (58 → 59): 행별 버튼 개수, 양 끝 비활성화, `↓`로 순서가 바뀌는지, `+`가 클릭한 행
  **다음**에 들어가는지, 편집한 순서대로 판독문 체인이 나가는지, `×`가 맞는 행을 지우는지.

## ver2.0013

- **크기를 입력하면 Increased / Decreased / Stable이 자동으로 채워진다.** 이전 크기와 오늘 크기가
  모두 있으면 계산해서 넣는다.
- 판정 기준 (2015 ATA, 사용자 제공 슬라이드 기준):
  - **Increased** — 2개 이상 축에서 `≥20%` 그리고 `≥2mm` 증가, **또는** 부피 `≥50%` 증가
  - **Decreased** — 위의 대칭 (감소 방향)
  - **Stable** — 그 외
- **부피 기준(≥50%)이 이번에 처음 들어갔다.** 그전에는 길이 기준만 계산했다. 부피는 기존
  타원체 공식(`AP×T×L×0.524`)을 쓰고, 세 축이 모두 있는 경우에만 적용한다.
- **판독의가 직접 고르면 그 선택이 이긴다.** 버튼을 누르면 `sizeChangeManual`이 서고, 이후
  크기를 다시 재도 자동값이 덮어쓰지 않는다. 같은 버튼을 한 번 더 눌러 해제하면 자동으로 돌아간다.
- 근거를 화면에 표시한다:
  `Δ max: +3 mm (+33%) · volume +78% · auto: Increased · significant growth (…)`
  수동으로 고른 상태에서는 `auto would say …`로 바뀌어, 계산과 판단이 다르다는 것이 보인다.
- 회귀 테스트 (57 → 58): 길이 기준 2축, 1축만 만족(부피도 미달), **부피 단독**(각 축 15%·1.5mm로
  길이 기준은 실패하지만 부피 +52%), 감소 대칭, 그리고 수동 선택이 재측정에도 살아남는지.

**주의 — 감소 기준은 가이드라인에 없다.** 2015 ATA는 growth만 정의한다. Decreased는 증가
기준을 대칭으로 뒤집은 이 도구의 관례이므로, 임상적으로 다르게 보시면 바꿔야 한다.
