...
- 현재 버전: **ver2.0017** (`Thyroid_KTIRADS_ver2.0017.html` — 스냅샷은 **저장소 루트**에 있다.
...
```bash
npm test                        # 60개 회귀 테스트, 전부 통과해야 정상
```
...
ver2.0017부터 이 게이트는 **각 섹션 헤더 안에** 있다 (실질/결절/림프절 제목줄). 별도 배너는 없다.
ver2.0010부터 **스터디가 완전히 비어 있는 동안에는 숨긴다.** 그 구간은 `Normal Study`
...
- 미착수(요청 없었음): JSON 백업/복원, 검증 무시하고 강제 생성, GitHub Actions CI
- 터치 드래그는 크기 이력 목록(ver2.0015)에만 있다. 다이어그램 마커 드래그는 마우스 기준이고
  실제 태블릿에서 확인된 적이 없다.

드래그를 새로 만들 때의 규칙 두 가지 (ver2.0015에서 둘 다 걸렸다):
`setPointerCapture`를 쓰지 말고 **이동/해제 리스너는 document에 걸 것** — 재렌더되는 카드 안의
노드는 캡처가 유지되지 않아 이벤트가 다른 행으로 샌다. 그리고 **드래그 중에는 재렌더하지 말고**
DOM만 옮긴 뒤 놓을 때 상태를 한 번 쓸 것.
