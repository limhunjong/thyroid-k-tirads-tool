...
- 현재 버전: **ver2.0007** (`Thyroid_KTIRADS_ver2.0007.html` — 스냅샷은 **저장소 루트**에 있다.
...
```bash
npm test                        # 55개 회귀 테스트, 전부 통과해야 정상
```
...
**아직 안 한 것 (3단계):** 컨트롤 높이 20종(34~64px)을 `--h-chip`/`--h-control`/`--h-nav` 3단으로.
과거 4번 재발한 "글자 잘림"이 전부 높이 CSS에서 나왔으므로 가장 조심할 단계다.
그 외: 상단바가 1024px에서 60px 넘친다 (ver2.0007 이전부터 있던 문제).
